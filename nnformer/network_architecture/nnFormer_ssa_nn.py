...
from nnformer.network_architecture.SSA import Block, Block_kv
...
class Patch_Expanding(nn.Module):
    def __init__(
        ...
        self.up=nn.ConvTranspose3d(dim, dim//2, pool_op_kernel_sizes, pool_op_kernel_sizes)  ## by du
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, S, H, W):
        for blk in self.blocks: ## 需要将block修改成3D版本，目前是2D版本
            x = blk(x, S, H, W)  
        
        if self.downsample is not None:
            x_down = self.downsample(x, S, H, W)
            # Ws, Wh, Ww = (S + 1) // 2, (H + 1) // 2, (W + 1) // 2
            s0, s1, s2 = self.pool_op_kernel_sizes
            Ws = S // s0 if s0 == 1 else (S + 1) // s0
            Wh = H // s1 if s1 == 1 else(H + 1) // s1
            Ww = W // s2 if s2 == 1 else (W + 1) // s2 

            # print(f"=======before: {S}-{H}-{W}, After: {Ws}-{Wh}-{Ww}")
...
class BasicLayer_up(nn.Module):
    def __init__(self,
        ...
        self.blocks.append(
            Block_kv(
                    ...
                    norm_layer=norm_layer,  ##直接换成drop_path[0]会不会有问题？
                    sr_ratio=sr_ratio
                )
        )
    ...
    def forward(self, x, skip, S, H, W):
        ...
        # S, H, W = S * 2, H * 2, W * 2   ## by du
        s0, s1, s2 = self.pool_op_kernel_sizes
        S, H, W = S * s0, H * s1, W * s2

        x = self.blocks[0](x, S, H, W, skip=skip, x_up=x_up) ##skip attention
...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        ...
        ### comment, get from plan.net_num_pool_op_kernel_sizes
...
class PatchEmbed_2x(nn.Module):
    ...
    def __init__(
        ...
        self.in_chans = in_chans
        self.embed_dim = embed_dim
        self.proj = project(in_chans, embed_dim, stride, 1, nn.GELU, nn.LayerNorm, True)
        if norm_layer is not None:
            self.norm = norm_layer(embed_dim)
        else:
            self.norm = None
...
class Encoder(nn.Module):
    def __init__(self,
        ...
        # self.encoder_pool_op_kernel_sizes = net_num_pool_op_kernel_sizes
        # self.encoder_conv_kernel_sizes = net_conv_kernel_sizes
        ...
        self.patch_embed = PatchEmbed(
            ...
            # stride=net_num_pool_op_kernel_sizes[0],
            stride1=net_num_pool_op_kernel_sizes[0],
            stride2=net_num_pool_op_kernel_sizes[1]
        )
        ...
        for i_layer in range(self.num_layers): ## self.num_layers=4
            layer = BasicLayer(
                ...
                pool_op_kernel_sizes=net_num_pool_op_kernel_sizes[i_layer + 2] if (i_layer < self.num_layers - 1) else None,
                conv_kernel_sizes=net_conv_kernel_sizes[i_layer + 2] if (i_layer < self.num_layers - 1) else None,
...
class Decoder(nn.Module):
    def __init__(self,
        ...
        # stochastic depth
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))]  # stochastic depth decay rule
        # dpr = tensor([0.0000, 0.0133, 0.0267, 0.0400, 0.0533, 0.0667, 0.0800, 0.0933, 0.1067,
        # 0.1200, 0.1333, 0.1467, 0.1600, 0.1733, 0.1867, 0.2000]) ## depths=[3, 4, 6, 3]
        ...
        for i_layer in range(self.num_layers)[::-1]:
            layer = BasicLayer_up(
                ...
                norm_layer=norm_layer,
                sr_ratio=sr_ratios[i_layer],
                pool_op_kernel_sizes=net_num_pool_op_kernel_sizes[i_layer], ##i_layer + 1
                conv_kernel_sizes=net_conv_kernel_sizes[i_layer], ##i_layer + 1
        ...
        # ### add by du
        # self.patch_expanding = Patch_Expanding_4x(
        #     dim=2*dim, 
        #     pool_op_kernel_sizes=pool_op_kernel_sizes,
        #     conv_kernel_sizes=pool_op_kernel_sizes,
        #     norm_layer=norm_layer
        # )
...
class final_patch_expanding(nn.Module):
    def __init__(
        ...
        super().__init__()
        # self.up=nn.ConvTranspose3d(dim,num_class,patch_size,patch_size) ## by du
...
class final_patch_expanding_nn(nn.Module):
    def __init__(
            self,
            dim,
            num_class,
            pool_kernel_size=[4,4,4],
...
class nnFormer(SegmentationNetwork):
    def __init__(self, crop_size=[64,128,128],
        ...
        self.decoder=Decoder(
            ...
            num_heads=num_heads[::-1][1:],
            mlp_ratios=mlp_ratios[::-1][1:],
            depths=depths[::-1][1:],
            sr_ratios=sr_ratios[::-1][1:],
            net_num_pool_op_kernel_sizes=net_num_pool_op_kernel_sizes[::-1],
            net_conv_kernel_sizes=net_conv_kernel_sizes[::-1],
        )
