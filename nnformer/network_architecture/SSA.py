...
class Mlp(nn.Module):
    ...
    def forward(self, x, S, H, W):
        x = self.fc1(x)
        x = self.act(x + self.dwconv(x, S, H, W))
        x = self.drop(x)
        x = self.fc2(x)
        x = self.drop(x)
        return x
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        if sr_ratio > 1:
            self.act = nn.GELU()
            if sr_ratio==8:
                self.sr1 = nn.Conv3d(dim, dim, kernel_size=8, stride=8)
                self.norm1 = nn.LayerNorm(dim)
                self.sr2 = nn.Conv3d(dim, dim, kernel_size=4, stride=4)
                self.norm2 = nn.LayerNorm(dim)
            if sr_ratio==4:
                self.sr1 = nn.Conv3d(dim, dim, kernel_size=4, stride=4)
                self.norm1 = nn.LayerNorm(dim)
                self.sr2 = nn.Conv3d(dim, dim, kernel_size=2, stride=2)
                self.norm2 = nn.LayerNorm(dim)
            if sr_ratio==2:
...
class Attention_kv(nn.Module):
    ...
    def forward(self, skip, x_up, S, H, W):
        B, N, C = x_up.shape
...
class Block_kv(nn.Module):
    ...
    def forward(self, x, S, H, W, skip=None, x_up=None):
        # 需要改成3D的
        # shortcut = x
        #### x=skip + x_up
        skip = self.norm1(skip)
        x_up = self.norm1(x_up)
        x = x + self.drop_path(self.attn(skip, x_up, S, H, W))
        x = x + self.drop_path(self.mlp(self.norm2(x), S, H, W))
        return x
...
class OverlapPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=7, stride=4, in_chans=3, embed_dim=768):
        ...
        self.S, self.H, self.W = img_size[0] // patch_size[0], img_size[1] // patch_size[1], img_size[2] // patch_size[1]
        self.num_patches = self.S * self.H * self.W
...
class Head(nn.Module):
    def __init__(self, num):
        ...
        stem = [nn.Conv3d(3, 64, 7, 2, padding=3, bias=False), nn.BatchNorm2d(64), nn.ReLU(True)]
        for i in range(num):
            stem.append(nn.Conv3d(64, 64, 3, 1, padding=1, bias=False))
            stem.append(nn.BatchNorm2d(64))
            stem.append(nn.ReLU(True))
        stem.append(nn.Conv3d(64, 64, kernel_size=2, stride=2))
...
class ShuntedTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stages):
            ...
            x, S, H, W = patch_embed(x)
            for blk in block:
                x = blk(x, S, H, W)
            x = norm(x)
            if i != self.num_stages - 1:
                # x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
                x = x.reshape(B, S, H, W, -1).permute(0, 4, 1, 2, 3).contiguous()
